...
- **ExportExcelUseCase** — generate / download `Tong_hop_hoa_don_T{month}_{year}.xlsx` (from RustFS or create from template)
...
s3://exports/Tong_hop_hoa_don_T{tháng}_{năm}.xlsx
```

**Monthly Excel file naming:**
- Pattern: `Tong_hop_hoa_don_T{N}_{year}.xlsx` (e.g., `Tong_hop_hoa_don_T4_2026.xlsx`)
- Tạo từ template nếu file chưa tồn tại
- File XLS được **append** mỗi khi confirm — không ghi đè toàn bộ
- Lưu lại RustFS sau mỗi append
